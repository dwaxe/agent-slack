...
- Prefer labeled links using Markdown (`[label](https://example.com)`) or Slack mrkdwn (`<https://example.com|label>`). Older `agent-slack` versions may leave a bare URL as plain text when a list causes native rich-text conversion, and compact read-back does not distinguish that text from a clickable link.
